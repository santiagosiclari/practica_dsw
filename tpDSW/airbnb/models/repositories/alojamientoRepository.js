...
export class AlojamientoRepository {
    ...
    async buscarConFiltros(filtros, page, limit) {
        const query = {};
        if(filtros.ciudad){ query['direccion.ciudad'] = filtros.ciudad }
        if (filtros.pais) query['direccion.pais'] = filtros.pais;
        ...
        const resultados = await this.model.find(query).skip(skip).limit(limit);
        const total = await this.model.countDocuments(query);

        return resultados.map(docToAlojamiento)
